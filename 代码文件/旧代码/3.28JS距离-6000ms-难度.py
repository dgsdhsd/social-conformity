# -*- codeing = utf-8 -*-
# @Time : 2025/3/28 13:25
# @Author : 星空噩梦
# @File ： 3.28JS距离-6000ms-难度.py
# @Software : PyCharm
...
for row in CONSISTENT:
    ...
    entropies_CONSISTENT.append(last_five)

entropies_NONCONSISTENT = []
for row in NONCONSISTENT:
    last_five = row[-5:]  # 提取最后 5 个数值
    entropies_NONCONSISTENT.append(last_five)
A_array=[]
B_array=[]
for now in entropies_CONSISTENT:
    all=sum(now)
    mm=[]
    for i in range(len(now)):
        mm.append(now[i]/all)
        A_array.append(mm)
for now in entropies_NONCONSISTENT:
    all=sum(now)
    mm=[]
    for i in range(len(now)):
        mm.append(now[i]/all)
...
#
# # 对三组互信息数据剔除异常值
# AA_js = remove_outliers(AA_js)
# BB_js = remove_outliers(BB_js)
# AB_js = remove_outliers(AB_js)
