# -*- codeing = utf-8 -*-
# @Time : 2025/8/11 13:43
# @Author : 星空噩梦
# @File ： 3_SGE_Nback.py
# @Software : PyCharm
'''
计算不同认知负荷的SGE的值，即1back和4back的值，采用独立t检验
采用划时间方式，主要计算前6000ms
'''
...
for ID in range(1, 31):
    ...
    for groupi in range(20):
        ...
        lists.append(
            lists[0] + lists[1] + lists[2] + lists[3] + lists[4] + lists[5] + lists[6] + lists[7] + lists[8] + lists[
                9] - lists[10] - lists[11] - lists[12] - lists[13])
        ...
        if groupi + 1 in [11, 2, 13]:
            NBACK_1.append(lists)  # CONSISTENT是1back，NONCONSISTENT是4back
        if groupi + 1 in [7, 18, 9, 20]:
            NBACK_4.append(lists)

entropies_NBACK_1 = []
for row in NBACK_1:
    last_five = row[-5:]  # 提取最后 5 个数值
    entropy = calculate_entropy(last_five)  # 计算信息熵
    entropies_NBACK_1.append(entropy)

entropies_NBACK_4 = []
for row in NBACK_4:
    last_five = row[-5:]  # 提取最后 5 个数值
    entropy = calculate_entropy(last_five)  # 计算信息熵
    entropies_NBACK_4.append(entropy)

# 1. 计算均值
mean_NBACK_1 = np.mean(entropies_NBACK_1)
mean_NBACK_4 = np.mean(entropies_NBACK_4)
std_NBACK_1 = np.std(entropies_NBACK_1)
std_NBACK_4 = np.std(entropies_NBACK_4)

# 2. 计算偏移值
offset = mean_NBACK_1 - mean_NBACK_4

# 3. 进行t 检验
t_statistic, p_value_t = ttest_ind(entropies_NBACK_1, entropies_NBACK_4)
# 输出结果
print("NBACK_1组均值:", mean_NBACK_1)
print("NBACK_4组均值:", mean_NBACK_4)
print("NBACK_1组标准差:", std_NBACK_1)
print("NBACK_4组标准差:", std_NBACK_4)
print("偏移值 (NBACK_1 - NBACK_4):", offset)
print("\nT 检验结果:")
