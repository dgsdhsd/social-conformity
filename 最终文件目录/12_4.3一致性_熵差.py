...
for row in CONSISTENT:
    ...
    pro1_3_consistent = [a + b for a, b in zip(pro1_3_consistent, last_five)]
...
for row in NONCONSISTENT:
    last_five = row[-5:]  # 提取最后 5 个数值
    pro1_3_nonconsistent = [a + b for a, b in zip(pro1_3_nonconsistent, last_five)]
    entropy = calculate_entropy(last_five)  # 计算信息熵
...
entropies_CONSISTENT2 = []
for row in CONSISTENT:
    last_five = row[-5:]  # 提取最后 5 个数值
    pro3_6_consistent = [a + b for a, b in zip(pro3_6_consistent, last_five)]
    entropy = calculate_entropy(last_five)  # 计算信息熵
    entropies_CONSISTENT2.append(entropy)

entropies_NONCONSISTENT2 = []
for row in NONCONSISTENT:
    last_five = row[-5:]  # 提取最后 5 个数值
    pro3_6_nonconsistent = [a + b for a, b in zip(pro3_6_nonconsistent, last_five)]
    entropy = calculate_entropy(last_five)  # 计算信息熵
    entropies_NONCONSISTENT2.append(entropy)


entropies_CONSISTENT_cha = np.array(entropies_CONSISTENT) - np.array(entropies_CONSISTENT2)
entropies_NONCONSISTENT_cha = np.array(entropies_NONCONSISTENT) - np.array(entropies_NONCONSISTENT2)

entropies_CONSISTENT=entropies_CONSISTENT_cha
entropies_NONCONSISTENT=entropies_NONCONSISTENT_cha
...
total = sum(pro1_3_consistent)
pro1_3_consistent = [x / total for x in pro1_3_consistent]

total = sum(pro1_3_nonconsistent)
pro1_3_nonconsistent = [x / total for x in pro1_3_nonconsistent]

total = sum(pro3_6_consistent)
pro3_6_consistent = [x / total for x in pro3_6_consistent]

total = sum(pro3_6_nonconsistent)
pro3_6_nonconsistent = [x / total for x in pro3_6_nonconsistent]

print("1-3秒的一致性行为：")
print(pro1_3_consistent)

print("1-3秒的非一致性行为：")
print(pro1_3_nonconsistent)

print("3-6秒的一致性行为：")
print(pro3_6_consistent)

print("1-3秒的一致性行为：")
print(pro3_6_nonconsistent)
